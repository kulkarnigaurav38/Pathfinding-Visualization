...
import { recursiveDivisionMaze } from "../maze/recursiveDivision.js";
import { verticalMaze } from "../maze/verticalMaze.js";
import { horizontalMaze } from "../maze/horizontalMaze.js";
...
export default class PathfindingVisualizer extends Component {
  ...
  render() {
    const { grid, mouseIsPressed, status, distanceToBeTraveled, algo } =
      this.state;
    let button_task = (
      <div className="rBtn">
        <div className="dropdown leftBtn">
          ...
          <div className="dropdown-content">
            <a onClick={() => this.setAlgoAstar()}> A * Search </a>{" "}
            <a onClick={() => this.setAlgoDijkstra()}> Dijkstra </a>
            <a onClick={() => this.setAlgoGreedyBFS()}> Greed BFS </a>
            <a onClick={() => this.setAlgoBiGreedy()}>
              {" "}
              Bidirectional Greed Search{" "}
            </a>{" "}
          </div>{" "}
        </div>{" "}
        <div className="rightBtn">
          <p className="buttonContainer" onClick={() => this.visualize()}>
            Start Visualizing!
          </p>{" "}
        </div>{" "}
      </div>
      ...
      <div className="textBox">
        ...
        <div>
          <div className="dropdown mazePick">
            ...
            <div className="dropdown-content">
              <a onClick={() => this.generateRandomMaze()}> Random Maze </a>{" "}
              <a onClick={() => this.generateRecursiveDivisionMaze()}>
                {" "}
                Recursive Division Maze{" "}
              </a>
              <a onClick={() => this.generateVerticalMaze()}>
                {" "}
                Vertical Devision Maze{" "}
              </a>{" "}
              <a onClick={() => this.generateHorizontalMaze()}>
                {" "}
                Horizontal Devision Maze{" "}
              </a>
            </div>{" "}
          </div>{" "}
          <p className="buttonRand2" onClick={() => this.ClearWalls()}>
            Clear Walls{" "}
          </p>{" "}
        </div>{" "}
        <div> {button_task} </div>
    ...
    if (status === "Calculating Shortest Path") {
      textBox = <div className="space"> </div>;
    } else if (status === "Shortest Path") {
      textBox = (
        <div>
          {" "}
          {button_task} <div className="spaceL1"> </div>{" "}
        </div>
      );
    }
    let heading;
    ...
    return (
      <div className="pathfindingVisualizer">
        ...
        <div className="visualGridContainer" id="gridBody">
          <table
            className="grid"
            style={{
              borderSpacing: "0",
          ...
          <footer>
            <p class="footer__author">
              Check out the source code at: &ensp;
              <a href="https://github.com/kulkarnigaurav38/Pathfinding-Visualization">
